...
def parse_psi4_out(oufiles_dir,xyz, structure, pattern="*.out", gro= False):
    ...
    return QMDataBase(molecule=molecule, positions=positions, topology=topology, structure=structure, angles=angles,
                      qm_energies=qm_energies, optimized=optimized,extracted_torsions=extracted_torsions, gro=gro) 
...
class QMDataBase(DataBase):
    ...
    def __init__(self,molecule, positions, topology, structure,  qm_energies,mm_energies=Quantity(), delta_energies=Quantity(), phis=None, angles=None, optimized=None, directions= None, steps=None, extracted_torsions=None,time=None, gro=False):
    ...
    def to_dataframe(self, psi4=True):
        ...
        if psi4:
            data = [(  self.qm_energy[i], mm_energy[i], delta_energy[i],
                    self.optimized[i]) for i in range(self.n_frames)]

            columns = [ 'QM energy (KJ/mol)', 'MM energy (KJ/mol)', 'Delta energy (KJ/mol)',
                       'Optimized']
            
        if self.phis != None:
            # print(self.phis)
            phi_df=pd.DataFrame.from_dict(self.phis)
            energy_data=[( np.round(self.qm_energy[i],4),  np.round(mm_energy[i],4) , np.round(delta_energy[i],4), self.optimized[i]) for i in range(self.n_frames)]
        ...
        energy_df = pd.DataFrame(energy_data, columns=columns)
    ...
    def slice(self, key, copy=True, slice_by_torsions=None):
        ...
        if copy:
            ...
            qm_energy = deepcopy(qm_energy-qm_energy.min())
            mm_energy = deepcopy(mm_energy-mm_energy.min())
            delta_energy = qm_energy - mm_energy
        ...
        newtraj = self.__class__( molecule= molecule,
            positions=xyz, topology=topology, structure=structure, directions=directions, steps=steps, extracted_torsions=d_name, phis=phis,
            qm_energies=qm_energy,mm_energies=mm_energy, delta_energies= delta_energy, optimized=optimized, angles=angles, time=time,gro=gro)
